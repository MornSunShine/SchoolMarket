...
import com.main.maomorn.tools.bottombar.scrollsweetness.BottomNavigationBehavior;
...
public class BottomBar extends FrameLayout implements View.OnClickListener, View.OnLongClickListener {
    ...
    public static BottomBar attachShy(CoordinatorLayout coordinatorLayout, View userContentView, Bundle savedInstanceState) {
        ...
        if (userContentView != null && coordinatorLayout.getContext()
                .getResources().getBoolean(R.bool.bottom_bar_is_tablet_mode)) {
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        if (mFragmentManager != null
            ...
            } else if (bottomBarFragment.getFragment() != null) {
                bottomBarFragment.getFragment().onSaveInstanceState(outState);
            }
        }
    }
    ...
    public void useDarkTheme() {
        if (!mIsDarkTheme && mItems != null && mItems.length > 0) {
            ...
            for (int i = 0; i < mItemContainer.getChildCount(); i++) {
                ...
                ((ImageView) bottomBarTab.findViewById(R.id.bottom_bar_icon))
    ...
    public void setTypeFace(String typeFacePath) {
        ...
        if (mItemContainer != null && mItemContainer.getChildCount() > 0) {
            for (int i = 0; i < mItemContainer.getChildCount(); i++) {
                View bottomBarTab = mItemContainer.getChildAt(i);
                TextView title = (TextView) bottomBarTab.findViewById(R.id.bottom_bar_title);
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        mDarkBackgroundColor = ContextCompat.getColor(getContext(), R.color.darkBackgroundColor);
        ...
        mInActiveColor = ContextCompat.getColor(getContext(), R.color.inActiveBottomBarItemColor);

        mScreenWidth = MiscUtils.getScreenWidth(mContext);
        mTwoDp = Utils.dpToPixel(mContext, 2);
        mTenDp = Utils.dpToPixel(mContext, 10);
        mMaxFixedItemWidth = Utils.dpToPixel(mContext, 168);
        mMaxInActiveShiftingItemWidth = Utils.dpToPixel(mContext, 96);
    }

    private void initializeViews() {
        mIsTabletMode = !mIgnoreTabletLayout &&
                mContext.getResources().getBoolean(R.bool.bottom_bar_is_tablet_mode);
        ViewCompat.setElevation(this, Utils.dpToPixel(mContext, 8));
        View rootView = View.inflate(mContext, mIsTabletMode ?
                        R.layout.bottom_bar_item_container_tablet : R.layout.bottom_bar_item_container,
                null);
        mTabletRightBorder = rootView.findViewById(R.id.tablet_right_border);

        mUserContentContainer = (ViewGroup) rootView.findViewById(R.id.user_content_container);
        mShadowView = rootView.findViewById(R.id.bottom_bar_shadow);

        mOuterContainer = (ViewGroup) rootView.findViewById(R.id.bottom_bar_outer_container);
        mItemContainer = (ViewGroup) rootView.findViewById(R.id.bottom_bar_item_container);

        mBackgroundView = rootView.findViewById(R.id.bottom_bar_background_view);
        mBackgroundOverlay = rootView.findViewById(R.id.bottom_bar_background_overlay);
    ...
    private void notifyRegularListener(OnTabClickListener listener, boolean isReselection, int position) {
        OnTabClickListener onTabClickListener = listener;
    ...
    private void notifyMenuListener(OnMenuTabClickListener listener, boolean isReselection, @IdRes int menuItemId) {
        OnMenuTabClickListener onMenuTabClickListener = listener;
        if (!isReselection) {
    ...
    private void updateItems(final BottomBarItemBase[] bottomBarItems) {
        ...
        for (BottomBarItemBase bottomBarItemBase : bottomBarItems) {
            ...
            if (mIsShiftingMode && !mIsTabletMode) {
                layoutResource = R.layout.bottom_bar_item_shifting;
            } else {
                layoutResource = mIsTabletMode ?
                        R.layout.bottom_bar_item_fixed_tablet : R.layout.bottom_bar_item_fixed;
            }

            View bottomBarTab = View.inflate(mContext, layoutResource, null);
            ImageView icon = (ImageView) bottomBarTab.findViewById(R.id.bottom_bar_icon);
        ...
        if (!mIsTabletMode) {
            int proposedItemWidth = Math.min(
                    Utils.dpToPixel(mContext, mScreenWidth / bottomBarItems.length),
                    mMaxFixedItemWidth
            );
    ...
    private void darkThemeMagic() {
        if (!mIsTabletMode) {
            ...
            mTabletRightBorder.setBackgroundColor(ContextCompat.getColor(mContext, R.color.tabletRightBorderDark));
        }
    }
    ...
    private void selectTab(View tab, boolean animate) {
        ...
        ImageView icon = (ImageView) tab.findViewById(R.id.bottom_bar_icon);
        TextView title = (TextView) tab.findViewById(R.id.bottom_bar_title);
    ...
    private void updateCurrentFragment() {
        if (!mShouldUpdateFragmentInitially && mFragmentManager != null
            ...
            if (newFragment.getFragment() != null) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT
            ...
            if (bottomBar.useTopOffset()) {
                ...
                if (statusBarResource > 0) {
                    ...
                    offset = Utils.dpToPixel(activity, 25);
                }
                ...
                if (!bottomBar.useOnlyStatusbarOffset()) {
                    ...
                    if (activity.getTheme().resolveAttribute(android.R.attr.actionBarSize, tv, true)) {
                        ...
                        offset += Utils.dpToPixel(activity, 56);
                    }
                }
